...
//  August 8, 2024 fixed to lognormal surv likelihood in ctl and rec_age = 4 + starting year (1977+age4) (OLD fixed 1979 for 1977+age2) 
//  October 2, 2024 fixed/ corrected the log normal survey likelihood equation
//==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+
DATA_SECTION
  !!CLASS ofstream evalout("evalout.prj");
// Read data from the control file
  !! ad_comm::change_datafile_name("goa_dusk.ctl");    //Read in phases, penalties and priors from "tem.ctl"
...
// Priors
  ...
  init_number ph_logsurv			// Use lognormal survey biomass likelihood = 2, normal =1
...
//     random_number_generator r(1000);
  ...
  else {      	N_proj(endyr+1,1)= value(mean(pred_rec(1977+recage,endyr-recage))); }
...
//  Next year's abundance
 ...
 if(mceval_phase()) {
  ...
  k=k+i;
     ...
     else { 	N_proj(i+1,1)= value(mean(pred_rec(1977+recage,endyr-recage))); }
...
// Calculate likelihood for survey biomass
  for (i=1; i<=nyrs_srv1; i++) {
	  ...
	  surv_like(1) += log(pow(log(1 + square(obs_srv1_se(i)) / square(obs_srv1_biom(i))), 0.5)) + 0.5 * square(log(obs_srv1_biom(i) / pred_srv1(i)) / pow(log(1 + square(obs_srv1_se(i)) / square(obs_srv1_biom(i))), 0.5)); //new likelihood for survey biomass
	  else 
 ...
 if(nyrs_srv2>0) {  for (i=1; i<=nyrs_srv2; i++) {
	  ...
	  surv_like(2) += log(pow(log(1 + square(obs_srv1_se(i)) / square(obs_srv1_biom(i))), 0.5)) + 0.5 * square(log(obs_srv1_biom(i) / pred_srv1(i)) / pow(log(1 + square(obs_srv1_se(i)) / square(obs_srv1_biom(i))), 0.5)); //new likelihood for survey biomass
	  else 
...
FUNCTION compute_spr_rates
  ...
  B40= SBF40*mean(pred_rec(1977+recage,endyr-recage));
...
// Output file (tem.rep) which is loaded into R for data outputs
  ...
  report << "1"  <<" "<<Like_L+Like_C     <<" " ; report << "Maturity_Likelihood"   << endl;
  ...
  report << "SigmaR: "<<sigr<< " Nat_Mort: "<<natmort<<" Survey_q: "<<q_srv1<<" LL Survey_q: "<<q_srv2<<" Spawning Per Recruit "<< " "<<SBF40<<" "<<SB0<<" Virgin SPR "<<a50<<" agemat_50"<<endl;
...
FUNCTION write_sumreport
  ...
  sumreport<<SB0*mean(pred_rec(1977+recage,endyr-recage))<<endl;
  sumreport<<"     B_40"     <<endl;
  sumreport<<B40<<endl;
  sumreport<<"     B_35"     <<endl;
  sumreport<<SBF35*mean(pred_rec(1977+recage,endyr-recage))<<endl;
  sumreport<<"     F_40"     <<endl;
  sumreport<<F40<<endl;
  sumreport<<"     F_35"     <<endl;
  sumreport<<F35<<endl;
...
// Function to write out data file for new Ianelli 2005 projection model....
 ...
 newproj <<"#_N_recruitment_years"<<endl<<endyr-recage-1981+1<< endl;
 newproj <<"#_Recruitment_start_at_1977_yearclass=1981_for_age_4_recruits"<<yy(1977+recage,endyr-recage)<<endl<<pred_rec(1977+recage,endyr-recage)<< endl;
...
// Executive Summary table values, rownames followed by values
  ...
  rreport<<SB0*mean(pred_rec(1977+recage,endyr-recage))<<endl;
  rreport<<B40<<endl;
  rreport<<SBF35*mean(pred_rec(1977+recage,endyr-recage))<<endl;
